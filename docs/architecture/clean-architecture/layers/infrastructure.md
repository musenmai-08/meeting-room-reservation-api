...
現在の API 実行時は、`routes.ts` で Prisma Repository を生成して各 route factory に注入しています。InMemory Repository は UseCase テスト用 Fake として残します。

## Prisma

Prisma は DB アクセスの詳細です。

Domain Entity や UseCase が Prisma の型に依存しないようにします。Prisma のモデルと Domain Entity の相互変換は Infrastructure 層の Prisma Mapper に閉じ込めます。
