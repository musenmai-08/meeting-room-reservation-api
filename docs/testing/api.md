...
- `GET /available-resources`
  ...
  - 有効な利用停止枠と重複するリソースは一覧に含まれない
- `POST /resource-unavailable-periods`
  - request body が正しい場合、利用停止枠を作成して 201 を返す
  - request body の必須項目が不足している場合、400 を返す
  - `resourceType` が許可された値でない場合、400 を返す
  - `startAt` / `endAt` が日時として不正な場合、400 を返す
  - 指定したリソースが存在しない場合、404 を返す
  - 既存予約と時間帯が重複する場合、409 を返す
  - 既存利用停止枠と時間帯が重複する場合、409 を返す
- `GET /resource-unavailable-periods`
  - query parameter が正しい場合、条件に一致する利用停止枠一覧を 200 で返す
  - `resourceType` / `status` が許可された値でない場合、400 を返す
  - `from` / `to` が日時として不正な場合、400 を返す
- `GET /resource-unavailable-periods/{resourceUnavailablePeriodId}`
  - 利用停止枠が存在する場合、詳細を 200 で返す
  - 利用停止枠が存在しない場合、404 を返す
- `PATCH /resource-unavailable-periods/{resourceUnavailablePeriodId}/cancel`
  - 利用停止枠をキャンセルして 200 を返す
  - 利用停止枠が存在しない場合、404 を返す
  - 利用停止枠がすでにキャンセル済みの場合、409 を返す
